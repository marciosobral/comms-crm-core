...
- **Audit**: mutations record an entry with `AuditService.record` (before/after), which feeds the sale and customer history. Each record also publishes a change event on `GET /events` (SSE); the web app refreshes the matching queries through `apps/web/src/lib/change-events.ts`, so a new cached entity needs an entry there.
